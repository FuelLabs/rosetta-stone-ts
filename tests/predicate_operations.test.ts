...
import { MultiSig, MultiSigFactory } from '../src/sway-api';
...
test('should complete full predicate workflow with real funding and spending', async () => {
  ...
  const predicate = new Predicate({
    bytecode: MultiSigFactory.bytecode,
    abi: MultiSig.abi,
    provider,
  ...
  // Create transaction request for the predicate transfer
  ...
  // Sign the transaction hash with the required wallets to create witness data
  console.log('Collecting signatures (2-of-3)...');
  const chainId = await provider.getChainId();
  const txHash = transactionRequest.getTransactionId(chainId);
  
  // Sign the transaction hash with wallet1 and wallet2
  const signature1 = await wallet1.signMessage(txHash);
  const signature2 = await wallet2.signMessage(txHash);
  ...
  const transferFromPredicateTx = await predicate.sendTransaction(transactionRequest);
  
  // Wait for the transaction to be confirmed
  const predicateTransferResult = await transferFromPredicateTx.waitForResult();
  ...
  console.log('✓ Predicate validation tested');
